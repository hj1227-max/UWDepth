...
def test_simple(args):
    ...
    with torch.no_grad():
        for idx, image_path in enumerate(paths):
            ...
            features,beta, B = encoder(input_image)
            outputs = depth_decoder(features,beta,B)
            ...
            depth_normalized = (depth_resized_np - depth_resized_np.min()) / (depth_resized_np.max() - depth_resized_np.min())
            depth_colormap = (depth_normalized * 255).astype(np.uint8)
            im_depth_linear = pil.fromarray(depth_colormap, mode='L')  
            name_dest_im_depth_linear = os.path.join(output_directory, "{}_depth_linear.jpeg".format(output_name))
            im_depth_linear.save(name_dest_im_depth_linear)

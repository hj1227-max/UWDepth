...
def profile_once(encoder, decoder, x):
    x_e = x[0, :, :, :].unsqueeze(0)
    x_d,beta, B = encoder(x_e)
    flops_e, params_e = profile(encoder, inputs=(x_e, ), verbose=False)
    flops_d, params_d = profile(decoder, inputs=(x_d,beta, B, ), verbose=False)
...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        with torch.no_grad():
            for data in dataloader:
                ...
                t1 = time_sync()
                features, beta, B = encoder(input_color)
                output = depth_decoder(features,beta, B)
                t2 = time_sync()
    ...
    if not opt.eval_stereo:
        gt_path = os.path.join(splits_dir, opt.eval_split, "gt_depth.npz")
    else:
